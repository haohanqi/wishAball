		...
		<Row justify="start" align="middle" style={{marginBottom:"40px"}}>
			<KeyWord>Key Words:</KeyWord>
			{
				getSelectFiled(cities).map((word) => {
					return <Col><KeyWord>{word}</KeyWord></Col>})
			} 
...
const ResultSection = ({ result, cities, sports, getSelectFiled, showResult}) => {
	...
	return (
		<>
		{
			...
			<ResultSectionWrapper className="result-wrapper">
				...
				<KeyWords cities = { cities } sports = { sports } getSelectFiled = { getSelectFiled } />
